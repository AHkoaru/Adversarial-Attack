...
# Get the directory containing the current file (ov-seg directory)
ovseg_dir = os.path.dirname(current_file_path)
# Get the parent directory (workspace directory)
workspace_dir = os.path.dirname(ovseg_dir)
...
# Insert ov-seg directory to sys.path
if ovseg_dir not in sys.path:
    sys.path.insert(0, ovseg_dir)
...
from evaluation import calculate_l0_norm, calculate_pixel_ratio, calculate_impact
from utils import save_experiment_results

# Import datasets
from dataset import ADESet, CitySet, VOCSet

class OVSegModelWrapper(torch.nn.Module):
    """
    Wrapper for OV-Seg model to handle input format mismatch and preprocessing.
    """
    def __init__(self, model, class_names, target_short_edge=640):
        super().__init__()
        self.model = model
        ...
        if isinstance(x, list):
            ...
            for item in x:
                if isinstance(item, dict):
                    # Already a dict
                    if "class_names" not in item:
                        item["class_names"] = self.class_names
                    if "meta" not in item:
                        item["meta"] = {"dataset_name": "ade20k_sem_seg_val"}
                    inputs.append(item)
                    ...
                    print(f"Warning: Unsupported item type in OVSegModelWrapper: {type(item)}")
...
class AttackRunner:
    def __init__(self, cfg, args):
        ...
        # Build Model using OV-Seg Trainer logic
        ...
        # Setup Class Names and Wrapper
        self.class_names = self.get_class_names(self.dataset_name)
        print(f"Loaded class names for {self.dataset_name}: {len(self.class_names)} classes")
        
        # Initialize Wrapper with target short edge (default 640 for OV-Seg)
        self.wrapped_model = OVSegModelWrapper(self.model, self.class_names, target_short_edge=640)
    ...
    def get_class_names(self, dataset_name):
        ...
        if dataset_name == 'cityscapes':
            ...
            try:
                ...
                # Fallback to standard ADE20K classes
                print("Warning: ADE20K class names not found in MetadataCatalog. Using generic placeholder.")
            ...
            raise ValueError(f"Unknown dataset for class names: {dataset_name}")
    ...
    def run_attack(self, image_tensor, label_tensor, attack_cfg):
        ...
        if self.attack_type == 'pixle':
            # Calculate patch size based on attack_pixel ratio
            ...
            attacker = Pixle(
                ...
                is_sed_model=True,  # Flag for Detectron2-based model
            ...
            # Map snapshots to milestone query counts
    ...
    def run(self):
        ...
        for i, sample in tqdm(enumerate(self.dataset), total=min(len(self.dataset), num_images)):
            ...
            # Fix for VOC2012: Shift labels to match 20-class model
        ...
        def run_eval(images, filenames, mode='gt'):
            ...
            if mode == 'gt':
                # Use Trainer's builder for GT evaluation
            ...
            # Unwrap DatasetEvaluators if needed
            ...
            with torch.no_grad():
                for img_np, fname in zip(images, filenames):
                    h, w = img_np.shape[:2]
                    # img_np is BGR, convert to RGB for model input
                    img_rgb_np = img_np[:, :, ::-1].copy()
                    img_tensor = torch.from_numpy(img_rgb_np).unsqueeze(0).permute(0, 3, 1, 2).float().to(self.device)
                    ...
                    eval_inputs = [{
                        "image": torch.as_tensor(np.ascontiguousarray(img_np.transpose(2, 0, 1))),
                        "height": h,
                        "width": w,
                        "file_name": fname_key
                    }]
        ...
        init_metrics_gt = extract_metrics(init_res_gt, self.class_names)
        
        print("Evaluating Initial State (Benign)...")
        init_res_benign = run_eval(img_list, filename_list, mode='benign')
        init_metrics_benign = extract_metrics(init_res_benign, self.class_names)
        ...
        for i in range(1, 6):
            ...
            metrics_gt = extract_metrics(res_gt, self.class_names)
            ...
            metrics_benign = extract_metrics(res_benign, self.class_names)
        ...
        # Post-processing to fix formatting
        try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="OV-Seg Attack Runner")
    ...
    parser.add_argument("--dataset-name", type=str, default="ade20k")
