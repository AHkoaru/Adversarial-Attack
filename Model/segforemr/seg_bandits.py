...
from Attacker.bandits import Bandits

from utils import label_to_train_id

class AttackConfig:
    epsilon: float = 0.05
    learning_rate: float = 0.01
    fd_eta: float = 0.1  # 유한 차분법을 위한 스텝 크기
    prior_size: int = 50  # 이전 그래디언트 활용을 위한 크기
    data_independent: bool = True  # 데이터 독립적 사전 정보 사용 여부
...
def sliding_window_inference(image, gt_mask, feature_extractor, model, mode, device, tile_size=(1024, 1024), stride=(512, 512)):
    ...
    # NES 대신 Bandits 공격 사용
    attack_config = AttackConfig()
    attacker = Bandits(feature_extractor, model, 
                   epsilon=attack_config.epsilon,
                   learning_rate=attack_config.learning_rate,
                   fd_eta=attack_config.fd_eta,
                   prior_size=attack_config.prior_size,
                   data_independent=attack_config.data_independent,
...
if __name__ == "__main__":
    ...
    print("결과가 'seg_bandits.json' 파일에 추가 저장되었습니다.")