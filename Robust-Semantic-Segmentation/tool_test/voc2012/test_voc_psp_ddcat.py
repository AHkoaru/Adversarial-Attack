...
robust_seg_dir = os.path.join(current_dir, '../..')
root_dir = os.path.join(current_dir, '../../..')

sys.path.append(robust_seg_dir)
...
def main():
    ...
    # Load VOC2012 dataset using the new VOCSet class
    test_data = VOCSet(dataset_dir=args.data_root, use_gt=True)
    ...
    if not args.has_prediction:
        ...
        test(test_data, model, args.classes, mean, std, args.base_size, args.test_h, args.test_w, args.scales, gray_folder, color_folder, colors, args.index_start, args.index_step)
    if args.split != 'test':
...
def test(test_dataset, model, classes, mean, std, base_size, crop_h, crop_w, scales, gray_folder, color_folder, colors, index_start, index_step):
    ...
    # Calculate end index
    if index_step == 0:
        index_end = len(test_dataset)
    else:
        index_end = min(index_start + index_step, len(test_dataset))
    
    for i in range(index_start, index_end):
        image, filename, target = test_dataset[i]
        image_name = filename.split('.')[0] if '.' in filename else filename
        color_path = os.path.join(color_folder, image_name + '.png')

        data_time.update(time.time() - end)
        
        # Apply transform logic (equivalent to transform.ToTensor())
        # Convert BGR to RGB, then to float32 and normalize to [0, 1]
        image = image[:, :, ::-1].astype(np.float32) / 255.0
        target = target.astype(np.int64)
        
        # Convert to tensor format (H, W, C) -> (C, H, W) for PyTorch
        image = np.transpose(image, (2, 0, 1))

        _, h, w = image.shape  # Now image is (C, H, W)
        ...
        for scale in scales:
            ...
            # Convert back to (H, W, C) for cv2.resize
            image_hwc = np.transpose(image, (1, 2, 0))
            image_scale = cv2.resize(image_hwc, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
        ...
        if ((i + 1) % 10 == 0) or (i + 1 == len(test_dataset)):
            logger.info('Test: [{}/{}] '
                        ...
                        'Batch {batch_time.val:.3f} ({batch_time.avg:.3f}).'.format(i + 1, len(test_dataset),
                                                                                    data_time=data_time,
...
def cal_acc(test_dataset, pred_folder, classes, names):
    intersection_meter = AverageMeter()
    union_meter = AverageMeter()
    target_meter = AverageMeter()

    for i in range(len(test_dataset)):
        _, filename, target = test_dataset[i]
        image_name = filename.split('.')[0] if '.' in filename else filename
        pred = cv2.imread(os.path.join(pred_folder, image_name+'.png'), cv2.IMREAD_GRAYSCALE)
        # target is already a numpy array from the dataset
        ...
        logger.info('Evaluating {0}/{1} on image {2}, accuracy {3:.4f}.'.format(i + 1, len(test_dataset), image_name+'.png', accuracy))
