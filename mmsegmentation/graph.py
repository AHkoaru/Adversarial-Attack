...
def process_single_image(args):
    ...
    # Dictionary to store iter_idx and is_success values
    iter_success_log = {}
    
    # Save original image as 0th result
    adv_img_bgr_list.append(img_tensor_bgr)
    for iter_idx in range(config["iters"]):
        current_query, adv_img_bgr, is_success = attack.perturb(img_tensor_bgr, gt_tensor)
        img_tensor_bgr = adv_img_bgr
        
        # Store iter_idx and is_success in dictionary
        iter_success_log[iter_idx] = is_success
    ...
    for i in range(config["iters"]):
        if i in iter_success_log:
            if iter_success_log[i] == False:
                failure_count += 1
        cumulative_failures.append(failure_count)
    
    # Debug print
    print(f"Total iterations: {config['iters']}, Total failures: {failure_count}")
    print(f"Sample success log (first 10): {dict(list(iter_success_log.items())[:10])}")
...
def main(config):
    ...
    # Calculate average cumulative failures across all images
    avg_cumulative_failures = np.mean(all_cumulative_failures, axis=0)
    
    # Create and save average cumulative failure count graph
    plt.figure(figsize=(10, 6))
    plt.plot(range(config["iters"]), avg_cumulative_failures, 'r-', linewidth=2)
    plt.xlabel('Iteration')
    plt.ylabel('Average Cumulative Failure Count')
    plt.title('Average Cumulative Failure Count Over Iterations')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, "average_cumulative_failures.png"), dpi=300)
    plt.close()
    ...
    final_results = {
        ...
        "Average Cumulative Failures": avg_cumulative_failures.tolist(),
    }
...
if __name__ == '__main__':
    ...
    config["base_dir"] = f"./data/{config['attack_method']}/graph/{config['dataset']}/{config['model']}"
