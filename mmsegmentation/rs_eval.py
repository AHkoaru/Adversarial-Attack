...
def get_completed_images(resume_dir):
    """resume_dir에서 이미 처리 완료된 이미지 이름 목록을 반환"""
    completed = set()
...
def main(config):
    ...
    # Load dataset (상대 경로를 절대 경로로 변환)
    data_dir = config["data_dir"]
    if not os.path.isabs(data_dir):
        # 상대 경로인 경우, rs_eval.py 기준으로 절대 경로 변환
    ...
    if config.get("resume_dir"):
        # Resume 모드: 기존 디렉토리 사용
        base_dir = config["resume_dir"]
        current_time = os.path.basename(base_dir)
        print(f"\n=== Resuming from: {base_dir} ===\n")

        # 완료된 이미지 목록 가져오기
        completed_images = get_completed_images(base_dir)
        print(f"Found {len(completed_images)} completed images")

        # 완료되지 않은 이미지만 필터링
        filtered_data = []
        for img, fname, gt in zip(dataset.images, dataset.filenames, dataset.gt_images):
            # 파일명에서 확장자 제거
            img_name = os.path.splitext(os.path.basename(fname))[0]
            if img_name not in completed_images:
                filtered_data.append((img, fname, gt))

        if filtered_data:
            dataset.images, dataset.filenames, dataset.gt_images = zip(*filtered_data)
            dataset.images = list(dataset.images)
            dataset.filenames = list(dataset.filenames)
            dataset.gt_images = list(dataset.gt_images)
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        print(f"Resume mode enabled: {args.resume}")
